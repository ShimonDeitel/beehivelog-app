...
struct ContentView: View {
    ...
    @State private var editingEntry: BeehiveLogEntry?
...
struct EntryEditView: View {
    ...
    let entry: BeehiveLogEntry?
    let onSave: (BeehiveLogEntry) -> Void
    ...
    init(entry: BeehiveLogEntry?, onSave: @escaping (BeehiveLogEntry) -> Void) {
        self.entry = entry
        self.onSave = onSave
        _title = State(initialValue: entry?.title ?? "")
        _date = State(initialValue: entry?.date ?? Date())
        _f1 = State(initialValue: entry?.hive ?? "")
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle(entry == nil ? "Add Inspection" : "Edit Inspection")
            .toolbar {
